...
### Added
- **Kéo vạch chia để đổi tỉ lệ hai pane.** Vùng chạm rộng cho ngón cái: vạch dày **22px** khi chưa chọn tài liệu tra cứu (trước là 5px — quá mảnh), **26px** khi đang có tài liệu (thanh mang nút "Đổi" cũng kéo được). **Tay-nắm** (pill nhũ-kem) nay mới vẽ — B4a cố ý chưa vẽ vì lúc đó kéo chưa tồn tại.
...
### Changed — sau QA huynh: thanh chia mỏng lại (38 → 26px)
- **BỎ tên tài liệu khỏi vạch chia.** Tên là thứ tự thêm ở B4b, không nằm trong yêu cầu nào; mà nội dung tài liệu đang hiện ngay bên dưới và Gú vừa tự tay chọn nó vài giây trước — giá trị thấp, giá phải trả là **12px chiều cao mỗi lần dùng split**. Vạch còn **tay-nắm + nút "Đổi"**, vùng chạm kéo vẫn **26px** (đủ ngón cái, không phải hạ xuống mức mảnh khó trúng). Kéo theo: bỏ luôn lượt đọc sidecar để lấy tên mỗi lần đổi tài liệu.
...
- Đo lại trên Flip 4: vạch **78 device-px = 26 CSS px** (trước 114). Kéo lên/xuống vạch dịch đúng theo tay; bấm "Đổi" ra đúng màn chọn và **tỉ lệ đã kéo được giữ nguyên** qua lần đổi.
...
- **Guard "chết cho đẹp" cho read-path mới** (2 lớp): (1) native `Saf.probeReadable` mở thử file (openInputStream + chạm 1 byte, try/catch Throwable) GỌI TRƯỚC fetch — vì nếu để fetch chạm file đã move/xóa, local-server Capacitor ném `FileNotFoundException` KHÔNG bắt trong `shouldInterceptRequest` → **CRASH cả app** (main-process, `onRenderProcessGone` không đỡ; đo thật trên Flip4). Probe reject → JS throw → empty-state panda, app sống. (2) guard `arrayBuffer` rỗng (quyền SAF thu hồi → local-server trả 200 body rỗng, không throw) → cũng throw → panda thay vì trang trắng im lặng.
- CHỈ đổi `readPdfBytes` (`safFile.ts`) + thêm `probeReadable` native — `PdfView` (windowing/pinch-zoom/khử-chớp), `ViewerPage`, `onPageChange`/nhớ-trang GIỮ NGUYÊN. Lưới OOM renderer (`onRenderProcessGone` → recreate) vẫn còn làm lớp đỡ cuối. Native `Saf.readFileBase64` giữ lại (không còn dùng cho PDF). Không đụng split-screen. Không dep/token mới. Verify Flip4: file thường + 64MB render; file move-xóa → **panda, app KHÔNG crash** (trước fix crash); 0 read-OOM.
