...
export function deriveLight(args: {
  ...
  // "Thấy Atomman" = thiết bị connected. offline ("chưa thấy") CHỈ khi thật sự không connected.
  if (!entry || entry.connected !== true) return 'offline';
  // Connected nhưng chưa biết tiến độ (completion lỗi/không có) → vẫn THẤY Atomman → không báo offline sai.
