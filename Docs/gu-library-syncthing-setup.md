...
> **Phạm vi tối thiểu để unblock M3:** chỉ cần **1 điện thoại dev + Atomman** ghép cặp + sync folder kho. Ghép nốt điện thoại 2 + tablet để *dùng thật* làm sau, không chặn code.
...
| Máy | Phần mềm | Lý do |
|---|---|---|
| Atomman (Windows, 24/7) | **Syncthing chạy như Windows service** (qua installer Bill Stewart) | Node neo phải sync kể cả khi chưa ai đăng nhập. SyncTrayzor chỉ chạy khi có user login → KHÔNG hợp vai node neo. |
...
## Phần A — Atomman (Windows): Syncthing chạy như service
...
4. **Ghi lại 3 thứ** (sẽ cần ở Phần C + nhập vào app sau):
   - **Device ID của Atomman:** GUI → Actions (góc phải) → Show ID. Đây là ID huynh sẽ chọn trong app Gú's Library là "Atomman".
   - **API key của Atomman:** GUI → Actions → Settings → tab GUI → mục **API Key**. (Mỗi máy có key riêng — key này là của *Atomman*.)
   - **GUI listen address:** mặc định `127.0.0.1:8384`.
5. Đặt **đường dẫn folder kho** trên Atomman (vd `D:\GuLibrary\kho`). Đây sẽ là folder Syncthing share.
...
## Phần C — Ghép cặp Atomman ↔ điện thoại

1. Trên **Atomman GUI** → Add Remote Device → dán **Device ID của điện thoại** → đặt tên (vd "Dev Phone") → Save.
2. Trên **điện thoại** sẽ hiện thông báo "Device muốn kết nối" → chấp nhận; hoặc Add Remote Device → dán **Device ID của Atomman** → đặt tên "Atomman".
...
2. Trên **điện thoại** sẽ hiện lời mời "Atomman muốn share folder gu-library-kho" → Accept → chọn đường dẫn lưu trên điện thoại (vd thư mục trong shared storage để sau này SAF của app trỏ vào).
3. Đợi sync. Bỏ thử vài file/thư mục mẫu vào `D:\GuLibrary\kho` trên Atomman → kiểm tra nó xuất hiện trên điện thoại.
...
## Phần E — Bật versioning trên Atomman (làm luôn, thỏa M8)

> Việc này thuộc milestone M8 (chống xóa nhầm lan truyền) nhưng là cấu hình Syncthing, làm luôn ở đây để khỏi quay lại.

1. Trên **Atomman GUI** → folder `gu-library-kho` → Edit → tab **File Versioning**.
...
4. **Chỉ cần bật ở Atomman** — nó là nơi giữ bản cũ tập trung. Khi lỡ xóa nhầm ở một máy, bản cũ moi lại từ folder versioning trên Atomman.

---

## Phần F — Nghiệm thu setup (trước khi gọi M3)

- [x] Atomman: Syncthing chạy như service, vẫn sync sau khi **đăng xuất / khởi động lại máy mà chưa login**.
- [x] Điện thoại ↔ Atomman: trạng thái **Connected**.
- [x] Bỏ 1 file vào `kho` ở Atomman → vài giây sau thấy trên điện thoại (và ngược lại).
- [x] Versioning bật trên Atomman: xóa thử 1 file → bản cũ còn trong folder versioning.
...
| **Đâu là Atomman** | App gọi REST liệt kê devices → huynh chọn "đây là Atomman" một lần, lưu Preferences. |
| **Folder ID của kho** | `gu-library-kho` (để app hỏi `/rest/db/completion?folder=gu-library-kho&device=<atomman>`). |
...
Lặp Phần B + C + D cho **điện thoại 2** và **tablet**: cài Syncthing-Fork → ghép cặp với Atomman → accept share folder `gu-library-kho`. Mỗi máy có API key riêng, nhập vào Gú's Library trên máy đó.
