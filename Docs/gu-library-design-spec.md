...
- Có sẵn 1 Atomman chạy 24/7 làm hạ tầng.
...
5. **Atomman là trung tâm xử lý nặng:** node neo đồng bộ + xưởng convert/extract + (sau này) auto-download. Điện thoại chỉ làm việc nhẹ.
...
### 3.1 Cơ chế
- **Syncthing** chạy ngầm trên cả 3 thiết bị + Atomman, đồng bộ folder kho.
  ...
  - **Atomman (Windows, 24/7):** Syncthing chạy như **Windows service** (vd qua installer Bill Stewart), KHÔNG dùng SyncTrayzor — service mới sync được cả khi chưa ai đăng nhập.
...
- **Atomman là node neo 24/7:** giải bài toán hai điện thoại không bao giờ bật cùng lúc — chúng đồng bộ gián tiếp qua con neo.
...
App đọc API trạng thái của Syncthing và hiển thị **góc phải header**. Mốc an toàn là **"máy này đã đẩy hết lên Atomman chưa"** (KHÔNG cố báo "cả 3 máy đã giống nhau chưa" — không trả lời thành thật được vì máy kia có thể đang tắt). Ba trạng thái:

| Trạng thái | Ý nghĩa | Điều kiện |
|---|---|---|
| ✓ Đã đồng bộ | An toàn đổi máy | Không còn file chờ + thấy Atomman |
| ⟳ Đang đẩy… | Đợi chút | Còn file đang truyền lên Atomman |
| ⚠ Chưa thấy Atomman | Data còn kẹt ở máy này, về nhà mới đẩy được | Ngoài mạng / Atomman tắt |
...
- App đọc REST API của Syncthing chạy trên **chính máy đó** — đèn báo "máy NÀY đã đẩy hết chưa" nên hỏi instance local, không phải instance Atomman.
...
- **Đâu là Atomman:** app gọi REST liệt kê devices → người dùng chọn "đây là Atomman" một lần, lưu Preferences.
- **Map điều kiện (field v2 thật):** "thấy Atomman" = `/rest/system/connections` báo device Atomman connected (v2 có primary/secondary — chỉ đọc `connected`); "đã đẩy hết" = `/rest/db/completion?folder=<id kho>&device=<atomman>` đạt 100% (`completion` là number). Poll định kỳ (~10s) đủ cho MVP, không cần event API.
...
```
kho/
  _inbox/                         # file gốc chờ Atomman xử lý (đường VÀO)
...
- File gốc đã vào kho (qua Share khi ở ngoài) nhưng **chưa qua Atomman → chưa có PDF + sidecar**.
- App đánh dấu trạng thái ⏳: ở cấp môn (badge) và cấp tài liệu (file mờ, có nhãn, chưa bấm xem được).
- Tự hoàn tất khi về mạng và Atomman xử lý xong.
...
- File gốc rơi vào `_inbox/`, **tên gắn tiền tố môn đích** (vd `[Tố tụng Hình sự] file.pdf`; "Chưa phân loại" → tiền tố `[Chưa phân loại]`) → Syncthing đẩy lên Atomman.
- **Tiền tố môn là interface M6↔M7:** app chỉ biết môn đích lúc Gú chọn ở sheet, nhưng Atomman (M7) mới là nơi đặt cặp PDF+JSON vào đúng môn sau convert. Tiền tố tải môn đích kèm file qua `_inbox/` để M7 đọc — KHÔNG đẻ file phụ, tái dùng đúng cơ chế đặt tên của `_print/` (mục 5b).
...
- Lúc ngồi máy tính: copy file thẳng vào `_inbox/` → Atomman tự nuốt.

### 5.3 Đường dự phòng: nút "+" trong app
- Mở app → Thêm → file picker → chọn môn. Không phải đường mặc định.

### 5.4 Xử lý ở Atomman (xưởng convert + extract) — *worker M7, đã chốt thiết kế*

Worker là **script Python** (PyMuPDF cho PDF kèm toạ độ text, python-docx, python-pptx — hệ parse tài liệu mạnh hơn Node; worker là tiến trình riêng trên Atomman, không chia code với app nên "cùng ngôn ngữ app" không phải lợi thế). **Không nằm trong app.**
...
> **Đánh đổi đã chấp nhận:** import "xịn" cần đi qua Atomman. Ở ngoài thì file gốc vào trạng thái ⏳, convert khi về nhà.
...
**Ràng buộc đã chốt:**
- Cờ "cần in" là ý định người dùng → **phải sync** nhưng **KHÔNG ghi vào sidecar JSON** (sidecar do Atomman sở hữu/sinh; app + worker cùng ghi một file là đúng cảnh báo mục 14.4). Lưu dạng metadata app-owned **per-file** (không file hàng đợi tập trung) — đúng triết lý "nhiều file nhỏ, không file bị nhiều máy tranh ghi".
- Gom = **copy**, không move (giữ bản gốc trong môn).
- Tên trong `_print/` phải gắn tiền tố môn để chống trùng giữa các môn.
- `_print/` được Syncthing sync ra cả 3 máy + Atomman → nhân bản tạm; **kỷ luật dọn sau khi in** (versioning mục 10 đỡ lỡ tay).

**Đường nâng cấp (A — Phase 3, không đập lại C):** Atomman watch `_print/` (y như watch `_inbox/`) và tự đẩy lên folder Drive "in" bằng rclone/Drive API. Lúc đó bước 3 (kéo tay) biến mất, thành no-touch. Cùng một folder outbox, chỉ thêm chân Drive cho Atomman. Việc Atomman nói chuyện Drive vốn đã thuộc Phase 3 (off-site backup) — chân Drive cho outbox đi in **gộp chung** vào năng lực Drive đó, không phát sinh hạ tầng mới.
...
1. **Header:** tên "Gú's Library" (serif, đậm, nâu đậm `#553B08`) bên trái; **đèn sync dạng pill có CHỮ** bên phải (không chỉ chấm màu) — icon + nhãn đọc được: ✓ "Đã đồng bộ" (xanh trầm), ⟳ "Đang đẩy…" (nâu/amber), ⚠ "Chưa thấy Atomman" (đỏ trầm). Đổi cả màu + chữ theo trạng thái.
...
- **Rủi ro thật không phải hỏng ổ** (đã có 4 bản: 3 thiết bị + Atomman), mà là **xóa nhầm lan truyền** — Syncthing sync cả lệnh xóa ra mọi máy.
- **Bắt buộc:** bật **versioning của Syncthing trên Atomman** (giữ bản cũ file bị xóa/sửa, vd 30 ngày) → moi lại được khi lỡ tay.
- **Tùy chọn (Phase 3):** Atomman đẩy snapshot lên Google Drive định kỳ cho bản off-site.
...
- **Text-extraction layer** (ở Atomman) — nhiều "đầu đọc" (PDF/PPTX/Word) → một output chuẩn (text + cấu trúc + vị trí). Điểm mở rộng cho format mới.
- **Converter** (ở Atomman) — LibreOffice headless, mọi thứ → PDF.
...
Data model (folder=môn, lồng tự do, PDF + sidecar) · Sync (Syncthing + Atomman neo + đèn trạng thái) · Import (Share → Atomman convert + extract) · Viewer (PDF, nhảy trang) · Home lai · Backup versioning · Print outbox mức C (gom `_print/`, kéo Drive tay — mục 5b).
...
Auto-download elearning (Moodle API) · Nguồn luật công khai (vbpl.vn) · Off-site backup (Drive) · Print outbox mức A (Atomman tự đẩy `_print/` lên Drive — gộp vào năng lực Drive).
...
5. **Phụ thuộc Atomman cho import "xịn"** — chấp nhận được trong mô hình "về nhà tự sync", nhưng cần trạng thái ⏳ rõ ràng.
...
- ~~Cơ chế cụ thể Atomman watch `_inbox/`~~ → **ĐÃ CHỐT (trục 3 M7):** polling vài phút qua Windows Scheduled Task, stateless, stability check + lọc đuôi tạm (mục 5.4).
